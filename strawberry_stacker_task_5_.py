...
row_nos=[]   
def row_no_callback(msg3):
        global row_nos
        row_nos.append(int(msg3.data))
        #print(row_nos)

rospy.init_node('strawberry_stacker', anonymous=True)
rospy.Subscriber("/spawn_info",UInt8, row_no_callback)
...
class stateMoniter1:
    ...
    def pose_callback(self,msg1):
        global position0_x,position0_y,position0_z,position0_x1,position0_y1,position0_z1,position0_z2,position0_x2,position0_y2,position0_z3,position0_x3,position0_y3
        ...
        position0_x3 = round(msg1.pose.position.x,2)
        position0_y3 = round(msg1.pose.position.y,2)
...
class stateMoniter2:
    ...
    # Create more callback functions for other subscribers    
    def pose_callback(self,msg1):
        global position_x,position_y,position_z,position_x1,position_y1,position_z1,position_z2,position_x2,position_y2,position_z3,position_y3,position_x3
        ...
        position_x3 = round(msg1.pose.position.x,2)
        position_y3 = round(msg1.pose.position.y,2)
    ...
    def process(self,data):
        ...
        if ids != None:
            ...
            for ids, corner1 in Detected_ArUco_markers.items():
                ...
                return (center0_x, center0_y,ids)
        else:
            return "No_aruco"
...
def drone1():
    ...
    def get_setpoint(sp_y):
        ...
        if aruco_value_returned !='No_aruco':
            global center0_x, center0_y,box_id0
            center0_x,center0_y,box_id0=aruco_value_returned[0],aruco_value_returned[1],aruco_value_returned[2]

            #print("can land")
            box_x,box_y,box_z = round(position0_x2+(position0_z2*math.tan(math.radians(40))/200)*(center0_x-200),2),sp_y,position0_z3
            box_x1,box_y1,box_z1 = round(position0_x2+(position0_z2*math.tan(math.radians(40))/200)*(center0_x-200)),sp_y,position0_z

            while (position0_x3!=box_x):
    ...
    def land_drone_ofb(sp_x,sp_y,sp_z):
        while (position0_z1!=round(sp_z,1)):
            pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= sp_x, sp_y, sp_z-0.2
            local_pos_pub.publish(pos)
            rate.sleep()
            continue
        #delaying_drone(time=2) 
    ...
    def correcting_box_pos(sp_x,sp_y,sp_z):
        ...
        move_to_setpoint(round(sp_x,1),round(sp_y,1),round(sp_z,1), accuracy=True)
        while(True):
        ...
        while (position0_z1!=0.0):
            pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= box_x,box_y,-0.2
            local_pos_pub.publish(pos)
            rate.sleep()

            if str(gripper_bool_value0)=='True':
                break
            else:
                continue 
    ...
    while not rospy.is_shutdown():
        ...
        #global count_red_box, count_blue_box
        global prev_id
        count_red_box=0
        count_blue_box=0

        move_to_setpoint(0,0,3)
        ...
        while(row_nos!=[]):
            ...
            #move_to_setpoint(0,y,5)
            move_to_setpoint(0.0,y,3.0,accuracy=True)
            drone_search_box(25.0,y,3.0)

            if box_id0==2 and count_blue_box <=10:
                truck_box_z=1.70
                #[round(14.7-d1_intial[0],1),-7.4-d1_intial[1],1.7]
                land_box_x=14+0.85*2-d1_intial[0] #13.85
                land_box_y=-7.4+1.23*((count_blue_box+2)//4)-d1_intial[1]
                land_box_z=6.0
                count_blue_box+=2

            elif box_id0==1 and count_red_box<=10:
                truck_box_z=1.70
                #[round(57.35-d1_intial[0],1),round(64.75-d1_intial[1],1),1.7]
                land_box_x=56.7+0.85*2-d1_intial[0] 
                land_box_y=64.75+1.23*((count_red_box+2)//4)-d1_intial[1]
                land_box_z=6.0
                count_red_box+=2
            ...
            if box_id0==2 and count_blue_box>10:
                truck_box_z=1.80
                #[round(14.7-d1_intial[0],1),-7.4-d1_intial[1],1.7]
                land_box_x=14+0.85*(count_blue_box%4)-d1_intial[0] #13.85
                land_box_y=-7.4+1.23*(count_blue_box//4-3)-d1_intial[1]
                land_box_z=6.0
                count_blue_box+=2
                ...
                truck_box_z=1.80
                #[round(57.35-d1_intial[0],1),round(64.75-d1_intial[1],1),1.7]
                land_box_x=56.7+0.85*(count_red_box%4)-d1_intial[0] #13.85
                land_box_y=64.75+1.23*(count_red_box//4-3)-d1_intial[1]
                land_box_z=6.0
                count_red_box+=2       

            move_to_setpoint(round(land_box_x),round(land_box_y),round(land_box_z))       

            while (position0_x1!=round(land_box_x,1)):
                pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= land_box_x,land_box_y,land_box_z
                local_pos_pub.publish(pos)
                rate.sleep()
                continue

            land_drone_ofb(land_box_x,land_box_y,truck_box_z)

            ofb_ctl.gripper(value=False)

            move_to_setpoint(round(land_box_x),round(land_box_y),6.0)    
...
def drone2():
    ...
    def get_setpoint(sp_y):
        ...
        if aruco_value_returned !='No_aruco':
            global center_x, center_y,box_id
            center_x,center_y,box_id=aruco_value_returned[0],aruco_value_returned[1],aruco_value_returned[2]

            #print("can land")
            box_x,box_y,box_z = round(position_x2+(position_z2*math.tan(math.radians(40))/200)*(center_x-200),2),sp_y,position_z3
            box_x1,box_y1,box_z1 = round(position_x2+(position_z2*math.tan(math.radians(40))/200)*(center_x-200)),sp_y,position_z

            while (position_x3!=box_x):
                pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= box_x,box_y,box_z
                local_pos_pub.publish(pos)
                rate.sleep()
                continue
    ...
    def land_drone_ofb(sp_x,sp_y,sp_z):
        while (position_z1!=round(sp_z,1)):
            pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= sp_x, sp_y, sp_z-0.2
            local_pos_pub.publish(pos)
            rate.sleep()
            continue
    ...
    while not rospy.is_shutdown():
        ...
        while(row_nos!=[]):
            ...
            #move_to_setpoint(0,y,6)
            move_to_setpoint(0.0,y,3.0,accuracy=True)
            # move_to_setpoint(0.0,y,3.0)
            drone_search_box(25.0,y,3.0)

            if box_id==2 and count_blue_box<=11:
                truck_box_z=1.70
                #[round(14.7-d1_intial[0],1),-7.4-d1_intial[1],1.7]
                land_box_x=14+0.85*(count_blue_box%4)-d2_intial[0] #13.85
                land_box_y=-7.4+1.23*(count_blue_box//4)-d2_intial[1]
                land_box_z=6.0
                count_blue_box+=2

            elif box_id==1 and count_red_box<=11: 
                truck_box_z=1.70
                #[round(57.35-d1_intial[0],1),round(64.75-d1_intial[1],1),1.7]
                land_box_x=56.7+0.85*(count_red_box%4)-d2_intial[0] #13.85
                land_box_y=64.75+1.23*(count_red_box//4)-d2_intial[1]
                land_box_z=6.0
                count_red_box+=2
            ...
            if box_id==2 and count_blue_box>11:
                ...
                #[round(14.7-d1_intial[0],1),-7.4-d1_intial[1],1.7]
                land_box_x=14+0.85*(count_blue_box%4)-d2_intial[0] #13.85
                land_box_y=-7.4+1.23*(count_blue_box//4-3)-d2_intial[1]
                land_box_z=6.0
                count_blue_box+=2
                ...
                #[round(57.35-d1_intial[0],1),round(64.75-d1_intial[1],1),1.7]
                land_box_x=56.7+0.85*(count_red_box%4)-d2_intial[0] #13.85
                land_box_y=64.75+1.23*(count_red_box//4-3)-d2_intial[1]
                land_box_z=6.0
                count_red_box+=2        

            move_to_setpoint(round(land_box_x),round(land_box_y),round(land_box_z))    

            while (position_x1!=round(land_box_x,1)):
                pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= land_box_x,land_box_y,land_box_z
                local_pos_pub.publish(pos)
                rate.sleep()
                continue  
