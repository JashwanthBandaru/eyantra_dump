...
class stateMoniter:
    ...
    def pose_callback(self,msg1):
        ...
        position_x = round(msg1.pose.position.x,1)
        position_y = round(msg1.pose.position.y,1)
        position_z = round(msg1.pose.position.z,1)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if [position_x,position_y,position_z]==setpoints[0]:
        ...
        if [position_x,position_y,position_z]==setpoints[1]:
            quaternion = quaternion_from_euler(0, 0, 90)
            pos.pose.orientation.x, pos.pose.orientation.y, pos.pose.orientation.z, pos.pose.orientation.w = quaternion[0],quaternion[1],quaternion[2],quaternion[3]
        ...
        if [position_x,position_y,position_z]==setpoints[3]:
            quaternion = quaternion_from_euler(0, 0, 270)
            pos.pose.orientation.x, pos.pose.orientation.y, pos.pose.orientation.z, pos.pose.orientation.w = quaternion[0],quaternion[1],quaternion[2],quaternion[3]
            pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= setpoints[0][0],setpoints[0][1],setpoints[0][2]
            vel.linear.x=0
            vel.linear.y=-5   
        ...
        if count==2 and [position_x,position_y,position_z]==setpoints[0]:
            quaternion = quaternion_from_euler(0, 0, 359)
            pos.pose.orientation.x, pos.pose.orientation.y, pos.pose.orientation.z, pos.pose.orientation.w = quaternion[0],quaternion[1],quaternion[2],quaternion[3]
            vel.linear.x=0
            vel.linear.y=0 
            ...
            # pos.pose.position.x, pos.pose.position.y, pos.pose.position.z= [0.0,0.0,0.0]
            # vel.linear.x=0
            # vel.linear.y=0  
        # if count==2 and [position_x,position_y,position_z]==[0.0,0.0,0.0]:
        #     while stateMt.state.armed:
        #         ofb_ctl.setArm(value=False)
        #         rate.sleep()
        #     print("Disarmed!!")
        #     break
